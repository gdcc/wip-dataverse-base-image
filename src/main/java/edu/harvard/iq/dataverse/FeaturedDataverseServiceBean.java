...
public class FeaturedDataverseServiceBean {
    ...
    public List<Dataverse> findByDataverseIdQuick(Long dataverseId) {
         ...
         try {
             ...
             searchResults = em.createNativeQuery("SELECT d.id, d.alias, d.name, d.name_hu FROM dataverse d, DataverseFeaturedDataverse f WHERE f.featureddataverse_id = d.id AND f.dataverse_id = "+dataverseId+" order by f.displayOrder").getResultList();
         } catch (Exception ex) {
             return null;
         }
         ...
         for (Object[] result : searchResults) {
            ...
            if(result[3] != null){
                dataverse.setName_hu((String)result[3]);
            }
