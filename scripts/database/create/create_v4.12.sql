...
CREATE TABLE DATASETEXTERNALCITATIONS (ID BIGINT NOT NULL, CITEDBYURL VARCHAR(255) NOT NULL, DATASET_ID BIGINT NOT NULL, PRIMARY KEY (ID));
...
CREATE TABLE DATAVERSE (ID BIGINT NOT NULL, AFFILIATION VARCHAR(255), ALIAS VARCHAR(255) NOT NULL UNIQUE, DATAVERSETYPE VARCHAR(255) NOT NULL, description TEXT, description_hu TEXT, FACETROOT BOOLEAN, GUESTBOOKROOT BOOLEAN, METADATABLOCKROOT BOOLEAN, NAME VARCHAR(255) NOT NULL, NAME_HU VARCHAR(255), PERMISSIONROOT BOOLEAN, TEMPLATEROOT BOOLEAN, THEMEROOT BOOLEAN, DEFAULTCONTRIBUTORROLE_ID BIGINT, DEFAULTTEMPLATE_ID BIGINT, PRIMARY KEY (ID));
...
CREATE TABLE VARIABLEMETADATA (ID  SERIAL NOT NULL, INTERVIEWINSTRUCTION TEXT, ISWEIGHTVAR BOOLEAN, LABEL TEXT, LITERALQUESTION TEXT, NOTES TEXT, UNIVERSE VARCHAR(255), WEIGHTED BOOLEAN, DATAVARIABLE_ID BIGINT NOT NULL, FILEMETADATA_ID BIGINT NOT NULL, WEIGHTVARIABLE_ID BIGINT, PRIMARY KEY (ID));
CREATE INDEX INDEX_VARIABLEMETADATA_datavariable_id ON VARIABLEMETADATA (datavariable_id);
CREATE INDEX INDEX_VARIABLEMETADATA_filemetadata_id ON VARIABLEMETADATA (filemetadata_id);
CREATE INDEX INDEX_VARIABLEMETADATA_datavariable_id_filemetadata_id ON VARIABLEMETADATA (datavariable_id, filemetadata_id);
...
CREATE TABLE VARGROUP (ID  SERIAL NOT NULL, LABEL TEXT, FILEMETADATA_ID BIGINT NOT NULL, PRIMARY KEY (ID));
CREATE INDEX INDEX_VARGROUP_filemetadata_id ON VARGROUP (filemetadata_id);
...
CREATE TABLE CATEGORYMETADATA (ID  SERIAL NOT NULL, WFREQ FLOAT, CATEGORY_ID BIGINT NOT NULL, VARIABLEMETADATA_ID BIGINT NOT NULL, PRIMARY KEY (ID));
CREATE INDEX INDEX_CATEGORYMETADATA_category_id ON CATEGORYMETADATA (category_id);
CREATE INDEX INDEX_CATEGORYMETADATA_variablemetadata_id ON CATEGORYMETADATA (variablemetadata_id);
...
CREATE TABLE VARGROUP_DATAVARIABLE (VarGroup_ID BIGINT NOT NULL, varsInGroup_ID BIGINT NOT NULL, PRIMARY KEY (VarGroup_ID, varsInGroup_ID));
...
ALTER TABLE VARIABLEMETADATA ADD CONSTRAINT UNQ_VARIABLEMETADATA_0 UNIQUE (datavariable_id, filemetadata_id);
...
ALTER TABLE DATASETEXTERNALCITATIONS ADD CONSTRAINT FK_DATASETEXTERNALCITATIONS_DATASET_ID FOREIGN KEY (DATASET_ID) REFERENCES DVOBJECT (ID);
...
ALTER TABLE VARIABLEMETADATA ADD CONSTRAINT FK_VARIABLEMETADATA_DATAVARIABLE_ID FOREIGN KEY (DATAVARIABLE_ID) REFERENCES DATAVARIABLE (ID);
ALTER TABLE VARIABLEMETADATA ADD CONSTRAINT FK_VARIABLEMETADATA_FILEMETADATA_ID FOREIGN KEY (FILEMETADATA_ID) REFERENCES FILEMETADATA (ID);
ALTER TABLE VARIABLEMETADATA ADD CONSTRAINT FK_VARIABLEMETADATA_WEIGHTVARIABLE_ID FOREIGN KEY (WEIGHTVARIABLE_ID) REFERENCES DATAVARIABLE (ID);
...
ALTER TABLE DATASETMETRICS ADD CONSTRAINT FK_DATASETMETRICS_DATASET_ID FOREIGN KEY (DATASET_ID) REFERENCES DVOBJECT (ID);
...
ALTER TABLE VARGROUP ADD CONSTRAINT FK_VARGROUP_FILEMETADATA_ID FOREIGN KEY (FILEMETADATA_ID) REFERENCES FILEMETADATA (ID);
...
ALTER TABLE CATEGORYMETADATA ADD CONSTRAINT FK_CATEGORYMETADATA_VARIABLEMETADATA_ID FOREIGN KEY (VARIABLEMETADATA_ID) REFERENCES VARIABLEMETADATA (ID);
ALTER TABLE CATEGORYMETADATA ADD CONSTRAINT FK_CATEGORYMETADATA_CATEGORY_ID FOREIGN KEY (CATEGORY_ID) REFERENCES VARIABLECATEGORY (ID);
...
ALTER TABLE VARGROUP_DATAVARIABLE ADD CONSTRAINT FK_VARGROUP_DATAVARIABLE_varsInGroup_ID FOREIGN KEY (varsInGroup_ID) REFERENCES DATAVARIABLE (ID);
ALTER TABLE VARGROUP_DATAVARIABLE ADD CONSTRAINT FK_VARGROUP_DATAVARIABLE_VarGroup_ID FOREIGN KEY (VarGroup_ID) REFERENCES VARGROUP (ID);
